...
// T0: 元の配列のモノイド
// T1: T0に対する作用素モノイド
template <class T0, class T1> class BaseImplicitTreap {
  // T0上の演算、単位元
...
template <class T0, class T1> struct MinUpdateQuery : public BaseImplicitTreap<T0, T1> {
...
template <class T0, class T1> struct SumAddQuery : public BaseImplicitTreap<T0, T1> {
  using BaseImplicitTreap<T0, T1>::BaseImplicitTreap;
...
template <class T0, class T1> struct MinAddQuery : public BaseImplicitTreap<T0, T1> {
  using BaseImplicitTreap<T0, T1>::BaseImplicitTreap;
...
template <class T0, class T1> struct MaxAddQuery : public BaseImplicitTreap<T0, T1> {
  using BaseImplicitTreap<T0, T1>::BaseImplicitTreap;
...
template <class T0, class T1> struct SumUpdateQuery : public BaseImplicitTreap<T0, T1> {
  using BaseImplicitTreap<T0, T1>::BaseImplicitTreap;
...
template <class T0> struct SumAffineQuery : public BaseImplicitTreap<T0, pair<T0, T0>> {
...
template <class T> struct MinmaxAffineQuery : public BaseImplicitTreap<pair<T, T>, pair<T, T>> {
...
int main() {
  ...
  MinAddQuery<ll, ll> bta;
  bta.set_by_vector(A);
  MaxAddQuery<ll, ll> btc;
  btc.set_by_vector(C);

  int ans = 0;
  rep(i, 0, N) {
    ll b = B[i];
    ll ida = bta.binary_search(0, bta.size(), b, false);
    if (ida == -1) {
      continue;
    }

    ll idc = btc.binary_search(0, btc.size(), b);
    if (idc == -1) {
      continue;
    }
    bta.erase(ida);
